...
def read_ini_secrets(secrets_file, strict=True) -> dict:
    ...
    * A path relative to deployed Python package. E.g. ``resource://tm/conf/test-settings.ini``

    :param secrets_file: URI like ``resource://tm/conf/test-settings.ini``
        ...
        for key, value in secrets_config.items(section):
            ...
            if value.startswith("$"):
                ...
                if not value and strict:
                    raise MissingSecretsEnvironmentVariable("Secrets key {} needs environment variable {} in file {} "
                                                            "section {}".format(key, environment_variable,
                                                                                secrets_file, section))
